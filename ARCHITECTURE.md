...
| 2026-06-12 | "A process is its page table + stack pointer" | A process IS its parked `registers` frame — all ~20 saved registers including RIP and RSP — plus a CR3 and a state. The frame is the identity; the stack pointer is just one field inside it | `process.h` |
| 2026-06-12 | "The context switch changes the page table and stack it works on" | Those are *consequences*. The mechanism is one struct copy — `*regs = processes[next].context` — replacing the frame that `iretq` is about to restore, inside the **timer** interrupt (vector 32) | `process.cpp` |
| 2026-06-12 | "Processes run for a couple of clock cycles" | A time slice is ~55 ms ≈ hundreds of **millions** of cycles. The letter-clumping is arithmetic: ~20 ms per print iteration ÷ 55 ms slice ≈ 2–3 letters per turn | `interrupts.cpp` |
| 2026-06-12 | "The kernel places the stack pointer on a ring 3→0 interrupt" | The kernel set `tss.rsp0` once at *boot*; at interrupt time the **CPU hardware** loads RSP from it automatically. And the *why* isn't "stop the user accessing things" — it's that the kernel can't trust the user's RSP (could be unmapped or aimed at kernel memory) | `gdt.cpp`, TSS |
| 2026-06-12 | "Not sure who reads a page-table entry" (THE big one) | The **MMU** (Memory Management Unit, hardware in the CPU) reads it — on **every single memory access**: every instruction fetch, every load, every store. Page tables aren't read by software; hardware walks them constantly. Unifying theme of the week: kernel *writes* rulebooks (page tables, GDT, IDT, TSS), hardware *reads & enforces* them nonstop | `vmm.cpp` |
